...
const categories: Record<string, string[]> = {
  'Izdavanje dokumenata': ['Lična karta', 'Pasoš', 'Izvod iz matične knjige'],
};

const QueueDocuments: React.FC = () => {
    ...
    const fetchAppointments = async () => {
      try {
        ...
        const filteredAppointments = response.filter((appointment: any) =>
          categories['Izdavanje dokumenata'].includes(
  ...
  const handleDelete = async (appointmentId: number) => {
    try {
      ...
      alert('Termin je uspješno završen.');
  ...
  const groupedAppointments = categories['Izdavanje dokumenata'].reduce(
    (acc, serviceSubType) => {
